...
const policyService = __importStar(require("../modules/policy/policy.service"));
const ruleGroupService = __importStar(require("../modules/ruleGroup/ruleGroup.service"));
const assignmentService = __importStar(require("../modules/assignment/assignment.service"));
...
async function createPlatformAdmin(email, password) {
    const passwordHash = await bcryptjs_1.default.hash(password, DEMO_PASSWORD_HASH_ROUNDS);
    const user = await user_model_1.User.create({ email, passwordHash, role: "PLATFORM_ADMIN", clientId: null });
    return String(user._id);
}
/** Creates a global policy and pushes it through submit-for-approval + approve, returning the active policy. */
async function publishGlobalPolicy(input, makerId, checkerId) {
...
async function main() {
    ...
    const alreadySeeded = await client_model_1.Client.findOne({ name: "Acme Retail" });
    if (alreadySeeded) {
    ...
    const acme = await client_model_1.Client.create({ name: "Acme Retail", status: "active", enabledStates: ["CA", "TX"] });
    const bolt = await client_model_1.Client.create({ name: "Bolt Logistics", status: "active", enabledStates: ["NY", "CA"] });
    console.log("Created 2 clients: Acme Retail, Bolt Logistics");
    await userService.createUser({ email: DEMO_USERS.acmeAdmin.email, password: DEMO_USERS.acmeAdmin.password, role: "CLIENT_ADMIN", clientId: String(acme._id) }, makerId);
    await userService.createUser({ email: DEMO_USERS.boltAdmin.email, password: DEMO_USERS.boltAdmin.password, role: "CLIENT_ADMIN", clientId: String(bolt._id) }, makerId);
    ...
    await assignmentService.createAssignment({ clientId: String(acme._id), ruleGroupId: acmeCaRuleGroup.ruleGroupId, targetType: "STATE", targetIds: ["CA"], priority: 0, effectiveFrom: new Date("2024-01-01") }, makerId);
    await assignmentService.createAssignment({ clientId: String(acme._id), ruleGroupId: acmeTxRuleGroup.ruleGroupId, targetType: "STATE", targetIds: ["TX"], priority: 0, effectiveFrom: new Date("2024-01-01") }, makerId);
    // emp-1001 gets their own rule group even though they're in CA — demonstrates EMPLOYEE beating STATE on resolve.
    await assignmentService.createAssignment({ clientId: String(acme._id), ruleGroupId: acmeTxRuleGroup.ruleGroupId, targetType: "EMPLOYEE", targetIds: ["emp-1001"], priority: 0, effectiveFrom: new Date("2024-01-01") }, makerId);
    await assignmentService.createAssignment({ clientId: String(bolt._id), ruleGroupId: boltNyRuleGroup.ruleGroupId, targetType: "STATE", targetIds: ["NY"], priority: 0, effectiveFrom: new Date("2024-01-01") }, makerId);
