...
export async function listPolicies(filter: ListPoliciesFilter, page: number, pageSize: number) {
  const query: Record<string, unknown> = { ...filter.tenantFilter };
  ...
  if (filter.state) query["jurisdiction.state"] = filter.state;
...
export async function archivePolicy(policyId: string, tenantFilter: Record<string, unknown>, actorId: string): Promise<PolicyDoc> {
  ...
  const full = await Policy.findById(doc._id);
  if (!full) throw new NotFoundError(`Policy ${policyId} not found`);
  const before = full.toObject();
  full.status = "archived";
  await full.save();
  await recordAudit({ entityType: "policy", entityId: policyId, action: "archive", actorId, before, after: full.toObject() });
  return full;
}
...
export async function clonePolicy(
  ...
  const source = await Policy.findOne({ policyId, scope: "global" }).lean();
  if (!source) throw new NotFoundError(`Global policy ${policyId} not found`);
